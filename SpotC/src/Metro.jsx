...
const Metro = () => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          const data = doc.data();
          // Vérifier que le createdAt existe (il peut être null pendant une fraction de seconde lors de la création)
          if (data.createdAt) {
            const createdAtDate = data.createdAt.toDate();
            const diffMinutes = (now - createdAtDate) / (1000 * 60);

            // Ne garder que les signalements datant de moins de 30 minutes
            if (diffMinutes <= 30) {
              // S'il y a plusieurs signalements pour une même station, on garde le plus récent
              validSignalements[data.station] = data;
            }
          }
        });
  ...
  // Vérifie si la station a un signalement actif
  const hasIncident = (station) => {
    return !!activeSignalements[station];
  };

  // Récupère les données du signalement pour une station
  const getIncidentData = (station) => {
    return activeSignalements[station];
  };
    ...
    <div style={styles.container}>
      <header style={styles.header}>
        ...
        <p style={styles.subtitle}>
          Consultez les correspondances et signalements en temps réel
        </p>
      </header>
        ...
        {metroLines.map((line) => (
          <button
            ...
            style={{
              ...
              borderBottom:
                activeLine.name === line.name
                  ? `3px solid ${line.color}`
                  : "3px solid transparent",
              color: activeLine.name === line.name ? "white" : "#888",
            }}
          >
            <div
              style={{ ...styles.lineCircle, backgroundColor: line.color }}
            ></div>
            {line.name.split(" ")[1]}
          </button>
        ))}
            ...
            return (
              ...
              >
                <span
                  ...
                  style={{
                    color: incident ? "#EF4444" : activeLine.color,
                    fontSize: "18px",
                    marginRight: "12px",
                  }}
                >
                  {incident ? "warning" : "radio_button_checked"}
                </span>
                <span style={{ color: incident ? "#EF4444" : "white" }}>
                  {station}
                </span>
              </div>
            );
          ...
          {selectedStation && metroDetails[selectedStation] ? (
            <div className="fade-in">
              ...
              <div
                style={{
                  ...styles.incidentBox,
                  borderColor: hasIncident(selectedStation)
                    ? "#EF4444"
                    : "#10B981",
                  backgroundColor: hasIncident(selectedStation)
                    ? "rgba(239, 68, 68, 0.05)"
              ...
              >
                <span
                  className="material-icons"
                  style={{
                    color: hasIncident(selectedStation) ? "#EF4444" : "#10B981",
                  }}
                >
                ...
                <div style={{ marginLeft: "12px" }}>
                  ...
                  <div
                    style={{
                      color: hasIncident(selectedStation)
                        ? "#EF4444"
                        : "#10B981",
                      fontWeight: "bold",
                      fontSize: "14px",
              ...
              <div style={styles.infoGrid}>
                <div style={styles.infoCard}>
                  <div style={styles.cardLabel}>
                    <span className="material-icons" style={styles.cardIcon}>
                      history_edu
                    </span>{" "}
                    HISTOIRE
                  </div>
                ...
                <div style={styles.infoCard}>
                  ...
                  <div style={styles.busContainer}>
                    {metroDetails[selectedStation].bus?.map((bus) => (
                      <div key={bus} style={styles.busBadge}>
                        {bus}
                      </div>
                    ))}
                  </div>
            ...
            <div style={styles.emptyState}>
              ...
              <p>
                Sélectionnez une station pour voir les détails et les
                signalements.
              </p>
            </div>
          )}
